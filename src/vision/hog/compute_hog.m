...
conf.dataDir = '/Users/eliabruni/data/esp/test/ouput/hog' ;
...
if ~exist(conf.vocabPath)
    ...
    for ii = 1:conf.numTrain
        ...
        descrs = anna_phog(fullfile(conf.calDir, images{ii}), 8, 360, 3, [1;22;1;30]) ;
...
for ii = 1:length(images)
    ...
    %im = imread(fullfile(conf.calDir, images{ii})) ;
    hists{ii - (iter * blockSize)} = getImageDescriptor(model, fullfile(conf.calDir, images{ii}));
...
% -------------------------------------------------------------------------
function hist = getImageDescriptor(model, I)
% -------------------------------------------------------------------------
...
% get hog
descrs = anna_phog(I, 8, 360, 3, [1;22;1;30]) ;
