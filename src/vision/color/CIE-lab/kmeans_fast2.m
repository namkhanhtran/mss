function [centers,mincenter,mindist,q2,quality] = kmeans_fast2(data,initcenters,method)
...
%        mobile = find(mindist > max(nndist(mincenter),ldist));
        mobile = (mindist > nndist(mincenter));
...
% the following is incorrect: for j = unique(mcm)'
            ...
            if ~any(track), continue; end
            alt = mdm(track) > lower(mobile&track,j);          
            if ~any(alt), continue; end
...
% redo may be empty, but we don't need to check this
            ...
            track2 = (mindist(track1) > centdist(mincenter(track1),j));
            track1 = track1&track2;
            if ~any(track1), continue; end
           
            % calculate exact distances to center j
            track4 = (lower(track1,j) < mindist(track1));
            if ~any(track4), continue; end
            track5 = track1&track4;
            jdist = calcdist(data(track5,:),centers(j,:));
            computed = computed + sum(track5,1);
...
%   [iteration toc nchanged computed size(diffj,2)]
    %disp(num2str([iteration toc nchanged computed]))
...
quality = mean(udist);
q2 = mean(udist.^2);
%disp(num2str([iteration toc quality q2 total]))
